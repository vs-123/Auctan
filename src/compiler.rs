...
impl Interpreter {
    pub fn interpret(source_code: Vec<String>, code: Vec<Node>) {
        ...
        println!("\n\nDEBUGGING:\nVARIABLES: {:?}", interpreter.defined_variables);
    }
    ...
    fn get_type_and_value(&mut self, node: Node) -> (ast::Type, String) {
        match node {
            ...
            Node::Str(le_str) => return (ast::Type::Str, le_str), // `le` doesn't signify anything here
            ...
            Node::Add(left_node, right_node) => {
                ...
                if left_type == ast::Type::Str {
                    ...
                    Interpreter::error(format!("Code:\n{} | {}\nProblem: `{}` is not a number.",
                        self.line_number,
                        self.source_code[self.line_number],
                        left
                    ));
                }
                ...
                if left_type == ast::Type::Num && right_type == ast::Type::Num {
                    return (ast::Type::Num, format!("{}", left.parse::<i32>().unwrap() + right.parse::<i32>().unwrap()));
    ...
    fn interpret_node(
        ...
        match node {
            Node::Assign(identifier, value) => {
                let (mut identifier_type, mut identifier_value) = self.get_type_and_value(*identifier);
                ...
                // Checking value
                if !vec![ast::Type::Num, ast::Type::Str, ast::Type::Identifier].contains(&value_type) {
                ...
                self.defined_variables.insert(identifier_value.to_string(), (value_type, value.to_string()));
            }
